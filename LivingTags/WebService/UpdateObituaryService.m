...
@implementation UpdateObituaryService

+(id)service
{
    static UpdateObituaryService *master=nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        master=[[UpdateObituaryService alloc] initWithService:WEB_SERVICE_UPDATE_OBITUARIES];
    });
    return master;
}
...
-(void)callUpdateObituaryServiceWithTKey:(NSString *)strTKey obituaryID:(NSString *)strObituaryID withCompletionHandler:(WebServiceCompletion)handler
{
    if (appDel.isRechable)
    {
        ...
        [arr addObject:[NSString stringWithFormat:@"obituary_id=%@",strObituaryID]];
                    ...
                    if (data.length>0)
                    {
                        ...
                        else
                        {
                            @try
                            {
                                ...
                                {
                                    NSDictionary *dictUser=[[responseDict objectForKey:@"response"] objectForKey:@"data"];
                                    handler(dictUser,NO,nil);
                                }
                                else
                                {
