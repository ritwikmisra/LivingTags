...
+(id)service
{
    ...
    dispatch_once(&onceToken, ^{
        master=[[LivingTagsSecondStepService alloc] initWithService:WEB_SERVICES_LIVING_TAGS_SECOND_STEP];
    });
    return master;
}
...
-(void)callSecondStepServiceWithDIctionary:(NSMutableDictionary *)dict UserID:(NSString *)strUserID livingTagsID:(NSString *)strLivingTagID withCompletionHandler:(WebServiceCompletion)completionHandler
{
    ...
    for (NSString *key in dict)
    {
        ...
        [dictParams setObject:[dict objectForKey:key] forKey:[NSString stringWithFormat:@"data[%@]",key]];
    }
    NSLog(@"%@",dictParams);
    if (appDel.isRechable)
    {
        ...
        NSDictionary *params1 = @{@"livingtag_id": strLivingTagID};
